...
int main()
{
    ...
    // 1/ MODEL MATRIX : used to transform objects to the global world space
    // Here we rotate the matrix on x so that it look's like it's laying on the floor
    glm::mat4 model = glm::mat4(1.0f);
    model = glm::rotate(model, glm::radians(-55.0f), glm::vec3(1.0f, 0.0f, 0.0f));

    // 2/ VIEW MATRIX
    // We will move our cam backwards into the scene so the object becomes visible
    glm::mat4 view = glm::mat4(1.0f);
    view = glm::translate(view, glm::vec3(0.0f, 0.0f, -3.0f)); // We move the objects on the opposite direction on which we want to move
